...
const firebaseConfigPath = path.join(projectRoot, 'config', 'firebase', 'firebase-applet-config.json');
...
if (!projectId) {
  console.error('Missing projectId in config/firebase/firebase-applet-config.json');
  process.exit(1);
}

if (!firestoreDatabaseId) {
  console.error('Missing firestoreDatabaseId in config/firebase/firebase-applet-config.json');
  process.exit(1);
}
...
if (mode !== 'hosting') {
  const syncResult = spawnSync('node', [path.join('scripts', 'firebase', 'generate-firestore-rules.mjs')], {
    cwd: projectRoot,
    stdio: 'inherit',
