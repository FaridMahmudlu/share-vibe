...
type AuditLogRecord = {
  id: string;
  createdAt?: Timestamp | { seconds?: number } | Date | number | null;
  [key: string]: any;
};

const getAuditLogTime = (record: AuditLogRecord) => {
  ...
  async getUserAuditLogs(userId: string, limit: number = 50): Promise<any[]> {
    try {
      ...
      return snapshot.docs
        .map((doc) => ({ id: doc.id, ...doc.data() } as AuditLogRecord))
        .sort((a, b) => getAuditLogTime(b) - getAuditLogTime(a))
        .slice(0, limit);
    } catch (error) {
